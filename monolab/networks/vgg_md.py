...
class VGGMonodepth(nn.Module):  # vgg version
    def __init__(self, num_in_layers, num_out_layers=2):
        super(VGGMonodepth, self).__init__()

        self.output_nc = num_out_layers

        self.downconv_1 = self.conv_down_block(num_in_layers, 32, 7)
    ...
    def disp_block(self, in_dim):
        ...
        disp_block = []
        disp_block += [
            nn.Conv2d(in_dim, self.output_nc, kernel_size=3, stride=1, padding=1),
            nn.Sigmoid(),
        ]  # h,w -> h,w
